"""绘制 2024 CUMCM A 题“板凳龙”问题 1 的论文插图。
...
def figure_1_parameter_definition() -> None:
    ...
    arrow_theta_start = THETA_INITIAL - 0.08
    arrow_theta_end = THETA_INITIAL - 0.22
    arrow_radius_start = B * arrow_theta_start + 0.18
    arrow_radius_end = B * arrow_theta_end + 0.18
    arrow_start = (
        arrow_radius_start * np.cos(arrow_theta_start),
        arrow_radius_start * np.sin(arrow_theta_start),
    )
    arrow_end = (
        arrow_radius_end * np.cos(arrow_theta_end),
        arrow_radius_end * np.sin(arrow_theta_end),
    )
    ...
    overview_limit = 9.55
    add_axis_arrows(ax_overview, (-overview_limit, 10.55), (-overview_limit, overview_limit))
    ax_overview.set_xlim(-overview_limit, 10.55)
    ...
    add_polar_arc(ax_local, 0.075, 0, theta_p, DARK_ORANGE, "$\\theta$", label_radius=0.050)
    ...
    save_figure(fig, "图01_阿基米德螺线参数示意图")
...
def _figure_1_bench_scale_legacy() -> None:
    ...
    _dimension_arrow(
        ...
        handle_distance,
        0.31,
        "$d_i=|P_iP_{i+1}|=1.65\\,\\mathrm{m}$（直线弦长）",
        DARK_PURPLE,
        label_above=True,
    )
    _dimension_arrow(
    ...
    save_figure(fig, "图01_板凳尺度与弦长约束_旧版")
...
def figure_3_velocity_transfer() -> None:
    ...
    labels = ["$P_i$", "$P_{i+1}$"]
    velocity_labels = ["$\\dot{\\mathbf{P}}_i$", "$\\dot{\\mathbf{P}}_{i+1}$"]
    ...
    for idx, (point, theta_value, color) in enumerate(zip(centers, theta_values, colors)):
        ...
        label_normal = np.array([-velocity_dir[1], velocity_dir[0]])
        velocity_label_pos = velocity_end + 0.18 * label_normal
        ax.text(
            velocity_label_pos[0],
            velocity_label_pos[1],
            velocity_labels[idx],
            color=color,
            ha="center",
            va="center",
    ...
    ax.text(
        ...
        "$\\Delta\\mathbf{P}_i\\cdot"
        "(\\dot{\\mathbf{P}}_{i+1}-\\dot{\\mathbf{P}}_i)=0$",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
    ...
    save_figure(fig, "图03_相邻把手速度传递关系示意图")
...
def main() -> None:
    ...
    OUTPUT_DIR = root / "图01_阿基米德螺线参数示意图"
    figure_1_parameter_definition()
    OUTPUT_DIR = root / "图02_相邻把手几何约束示意图"
    figure_2_chord_constraint()
    OUTPUT_DIR = root / "图03_相邻把手速度传递关系示意图"
    figure_3_velocity_transfer()
    print(f"已生成图 1～3：{root}")
