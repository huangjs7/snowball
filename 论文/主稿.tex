...
\begin{table}[H]
  ...
  \begin{tabular}{cp{0.58\textwidth}c}
    ...
    $\theta_i(t)$ & 第 $i$ 个把手中心的极角函数，$\theta_i:\mathbb{R}_{\geq0}\to\mathbb{R}_{\geq0}$ & $\mathrm{rad}$ \\
    $\bm P(\theta)$ & 螺线的位置向量函数，$\bm P:[0,\infty)\to\mathbb{R}^2$ & $\mathrm{m}$ \\
    $\bm P_i(t)$ & 第 $i$ 个把手中心的位置向量，$\bm P_i(t)=\bm P\!\left(\theta_i(t)\right)$ & $\mathrm{m}$ \\
    $d_i$ & 第 $i$ 与第 $i+1$ 个把手中心之间的固定直线距离 & $\mathrm{m}$ \\
    $S(\theta)$ & 从 $\theta=0$ 至 $\theta$ 的螺线累计弧长函数，$S(0)=0$ & $\mathrm{m}$ \\
    $\dot\theta_i(t)$ & 第 $i$ 个把手中心的角速度，$\dot\theta_i=\mathrm{d}\theta_i/\mathrm{d}t$ & $\mathrm{rad}\,\mathrm{s}^{-1}$ \\
    $v_i(t)$ & 第 $i$ 个把手中心的线速度大小，$v_i=\|\dot{\bm P}_i\|$ & $\mathrm{m}\,\mathrm{s}^{-1}$ \\
    \bottomrule
  \end{tabular}
\end{table}

其中，$\theta_i(t)$、$S(\theta)$ 和 $v_i(t)$ 为标量函数，
$\bm P(\theta)$ 与 $\bm P_i(t)$ 为二维列向量函数；上点表示对时间 $t$ 求导，
撇号表示对螺线参数 $\theta$ 求导。除特别说明外，$i=0,1,\ldots,223$；
涉及相邻把手的量 $d_i$ 时，$i=0,1,\ldots,222$。
...
\begin{equation}
  \theta_A=32\pi,\qquad
  \bm P_0(0)=\bm P(32\pi)=(8.8,0)^\mathsf{T}\ \mathrm{m}.
  \label{eq:q1-initial}
\end{equation}
...
\begin{equation}
  S(\theta)
  =\int_0^\theta \left\|\bm P'(u)\right\|\,\mathrm{d}u
  =\frac{b}{2}
  \left[
    \theta\sqrt{1+\theta^2}
...
\begin{equation}
  \bm P_0(t)=\bm P\bigl(\theta_0(t)\bigr).
...
两把手之间满足定长约束
\begin{equation}
  \left\|\bm P_{i+1}(t)-\bm P_i(t)\right\|=d_i,
  \qquad i=0,1,\ldots,222.
  \label{eq:q1-distance-constraint}
\end{equation}
为简化书写，在固定时刻 $t$ 下暂略函数自变量 $t$。设已知第 $i$ 个把手的
极角 $\theta_i$，将 $\bm P_i=\bm P(\theta_i)$、
$\bm P_{i+1}=\bm P(\theta_{i+1})$ 代入式
...
令
\begin{equation}
  \Delta\bm P_i=\bm P_{i+1}-\bm P_i,
\end{equation}
则式~\eqref{eq:q1-distance-constraint} 等价于
$\left(\Delta\bm P_i\right)^\mathsf{T}\Delta\bm P_i=d_i^2$。对时间求导可得
\begin{equation}
  \left(\Delta\bm P_i\right)^\mathsf{T}
  \left(\dot{\bm P}_{i+1}-\dot{\bm P}_i\right)=0.
...
\begin{equation}
  ...
  \end{bmatrix},
  \qquad
  \dot{\bm P}_i=\bm P'(\theta_i)\dot\theta_i,
...
\begin{equation}
  \dot\theta_{i+1}
  =\frac{
    \left(\Delta\bm P_i\right)^\mathsf{T}\bm P'(\theta_i)
  }{
    \left(\Delta\bm P_i\right)^\mathsf{T}\bm P'(\theta_{i+1})
  }\dot\theta_i,
  \qquad i=0,1,\ldots,222.
...
\end{equation}
负号表示极角随盘入过程减小。根据
$\|\bm P'(\theta)\|=b\sqrt{1+\theta^2}$，第 $i$ 个把手的线速度大小为
\begin{equation}
  v_i(t)
  =\left\|\dot{\bm P}_i(t)\right\|
  =b\sqrt{1+\theta_i^2(t)}\,
